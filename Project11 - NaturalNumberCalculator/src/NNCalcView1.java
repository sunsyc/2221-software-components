...
public final class NNCalcView1 extends JFrame implements NNCalcView {
    ...
    public NNCalcView1() {
        ...
        for (int i = 0; i < DIGIT_BUTTONS; i++) {
            this.bDigits[i] = new JButton(numbers[i]);
        }
        ...
        // initialize constants to arrange buttons in main panel
        final int seven = 7, six = 6, four = 4, three = 3, one = 1;
        // add buttons 7 to 9
        for (int i = seven; i < DIGIT_BUTTONS; i++) {
         ...
         */
        adjustPanel.add(buttonPanel);
        adjustPanel.add(sideButtonPanel);
        /*
    ...
    public void registerObserver(NNCalcController controller) {
        // register argument as observer/listener
        this.controller = controller;
    }

    @Override
    public void updateTopDisplay(NaturalNumber n) {
        // update top operand display
        this.tTop.setText(n.toString());
    }

    @Override
    public void updateBottomDisplay(NaturalNumber n) {
        // update bottom operand display
        this.tBottom.setText(n.toString());
    }

    @Override
    public void updateSubtractAllowed(boolean allowed) {
        // set subtract button enabled or disabled based on value of allowed,
        // the value of allowed is obtained from NNCalcController1.java
        if (allowed) {
    ...
    public void updateDivideAllowed(boolean allowed) {
        // set divide button enabled or disabled based on value of allowed
        if (allowed) {
    ...
    public void updatePowerAllowed(boolean allowed) {
        // set power button enabled or disabled based on value of allowed
        if (allowed) {
    ...
    public void updateRootAllowed(boolean allowed) {
        // set root button enabled or disabled based on value of allowed
        if (allowed) {
