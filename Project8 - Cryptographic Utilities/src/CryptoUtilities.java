...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
         ...
         */
        // if m is not zero, execute the block
        if (!m.isZero()) {
            NaturalNumber remainder = n.divide(m);
            // copy value of m in n & remainder in m
            n.copyFrom(m);
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        // create a copy of n called duplicate
        NaturalNumber duplicate = n.newInstance();
        duplicate.copyFrom(n);
        NaturalNumber remainder = duplicate.divide(two);
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
         ...
         */
        //create this copy to multiply n with itself a second time when p is odd
        NaturalNumber copy4n = n.newInstance();
        // create a copy of p to restore the value after method execution
        NaturalNumber copy4p = p.newInstance();
        copy4p.copyFrom(p);
        copy4n.copyFrom(n);
        NaturalNumber two = new NaturalNumber2(2);
        // if p is even and not zero
        if (isEven(p) && !p.isZero()) {
            // divide p by 2
            p.divide(two);
            powerMod(n, p, m);
            // create copy of n to multiply n with itself
            NaturalNumber tmpcopy = n.newInstance();
            tmpcopy.copyFrom(n);
            n.multiply(tmpcopy);
            // modulo division
            NaturalNumber remain = n.divide(m);
            // transfer result of modulo division in n
            n.transferFrom(remain);
        } else if (p.isZero()) {
            NaturalNumber one = new NaturalNumber2(1);
            // if p is zero, then n is 1 for any value of n
            n.copyFrom(one);
        } else {
            p.divide(two);
            powerMod(n, p, m);
            ...
            tmpcopy.copyFrom(n);
            n.multiply(tmpcopy);
            // use copy4n to multiply n with itself again
            n.multiply(copy4n);
            NaturalNumber remain = n.divide(m);
            n.transferFrom(remain);
        }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        boolean result = false;
        // constant to compare value of remainder after modulo division with one
        NaturalNumber one = new NaturalNumber2(1);
        // create copy of n to pass in powerMod as p
        NaturalNumber p = n.newInstance();
        // decrement p so it is <n-1
        p.copyFrom(n);
        p.decrement();
        powerMod(w, p, n);
        // divide w by n and store in remainder
        NaturalNumber remainder = w.divide(n);
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        // constant to generate 50 random numbers
        final int randomCounter = 50;
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            /*
             * 2 and 3 are primes
             */
            ...
            // create copy of n
            NaturalNumber copy = n.newInstance();
            for (int i = 1; i < randomCounter; i++) {
                copy.copyFrom(n);
                copy.subtract(one);
                // execute only if no is prime
                if (isPrime) {
                    NaturalNumber w = randomNumber(n);
                    // if w>1 and w<n-1 where n-1 is copy
                    if (w.compareTo(one) > 0 && w.compareTo(copy) < 0) {
                        isPrime = !isWitnessToCompositeness(w, n);
                    }
                } else {
                    // break the loop if number is composite
                    break;
                }
            }
        }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        boolean result = false;
        while (!result) {
            // if number is even, increment once
            if (isEven(n)) {
                n.increment();
            } else {
                // increment twice to skip even numbers
                n.increment();
                n.increment();
            }
            result = isPrime2(n);
