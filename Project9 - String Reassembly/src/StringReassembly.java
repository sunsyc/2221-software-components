...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
         ...
         */
        // substring of str2 from index overlap is concatenated at the end of str1
        String combo = str1.concat(str2.substring(overlap));
        /*
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
         ...
         */
        // declare set add to add elements in strSet
        Set<String> add = new Set1L<>();
        // declare set remove to remove elements from strSet
        Set<String> remove = new Set1L<>();
        for (String element : strSet) {
            // execute only if str is not a substring of each element of strSet
            if (element.indexOf(str) == -1) {
                // add the string to be added in strSet in add.
                add.add(str);
                // execute only if any element of strSet is a substring of str
                if (str.indexOf(element) != -1) {
                    // add the elemnet of strSet to be removed in set remove
                    remove.add(element);
                }
            }
        }
        // add elements from add and remove from remove
        strSet.add(add);
        strSet.remove(remove);
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // declare set remove to store elements to be removed
        Set<String> remove = new Set1L<>();
        // declare set to store strings read from each line
        Set<String> s = new Set1L<>();
        // execute till end of file is not reached
        while (!input.atEOS()) {
            String s1 = input.nextLine();
            // add string read to set s
            s.add(s1);
        }
        // compare each element of s with every other element of s
        // other than that element itself for substrings
        for (String element1 : s) {
            for (String element2 : s) {
                if (!element2.equals(element1)
                        && element2.indexOf(element1) != -1) {
                    // add elements that are a substring of e=other elements
                    remove.add(element1);
                }
            }
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        // initialize j to start printing string from index 0
        int j = 0;
        // index of substring at which ~ is found
        int i;
        for (i = 0; i < text.length(); i++) {
            if (text.charAt(i) == '~') {
                // print substring from index j to index i, where i is the
                // index at which ~ is found
                String sub = text.substring(j, i);
                out.println(sub);
                // change the value of j to i+1
                j = i + 1;
            }
        }
        // print the last substring
        out.println(text.substring(j, i));
    }
