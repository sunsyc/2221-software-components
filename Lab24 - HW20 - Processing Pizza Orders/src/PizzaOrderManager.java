...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        int price;
        String word;
        // execute till end of file not reached
        while (!input.atEOS()) {
            // store first line in s
            String s = input.nextLine();
            for (int i = 0; i < s.length(); i++) {
                // find ',' in s
                if (s.charAt(i) == ',') {
                    // convert string value of price into int
                    price = Integer.parseInt(s.substring(i + 1, s.length()));
                    // store substring from index 0 to i
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        int totalPrice = 0;
        String s = input.nextLine();
        totalPrice += sizePriceMap.value(s);
        s = input.nextLine();
        while (!s.equals("")) {
            // add price of toppings to total price
            totalPrice += toppingPriceMap.value(s);
            s = input.nextLine();
        }
        return totalPrice;
    }
