...
public final class NaturalNumberStaticOps {
    ...
    private static void printWithCommas(NaturalNumber n, SimpleWriter out) {
        ...
        final NaturalNumber thousand = new NaturalNumber2(1000);
        final NaturalNumber ten = new NaturalNumber2(10);
        NaturalNumber copy = new NaturalNumber2(n);
        if (copy.compareTo(thousand) < 0) {
            out.print(copy);
        } else {
            NaturalNumber remainder = copy.divide(thousand);
            printWithCommas(copy, out);
            if (remainder.compareTo(ten) < 0) {
                out.print(",00" + remainder);
            } else {
                out.print("," + remainder);
            }
        }
